...
from human_shadow.utils.image_utils import resize_image_to_rectangle, resize_img_to_square


def init_charuco_detector(checker_size, marker_size, ncols, nrows, dictionary):
    charuco_board = cv2.aruco.CharucoBoard((ncols, nrows), checker_size, marker_size, cv2.aruco.getPredefinedDictionary(dictionary))
...
def get_aruco_marker_pose(img, detector, camera_matrix, marker_id=0, marker_size_mm=45, aruco_dict_type=cv2.aruco.DICT_4X4_50):
    ...
    if ids is not None:
        for i, detected_id in enumerate(ids):
            if detected_id == marker_id:
                ...
                marker_size_m = marker_size_mm / 1000.0  # Convert size from mm to meters
                dist_coeffs = np.zeros(12)
                rvec, tvec, _ = cv2.aruco.estimatePoseSingleMarkers(corners, marker_size_m, camera_matrix, dist_coeffs)

                # Draw the axes on the image
                cv2.drawFrameAxes(img, camera_matrix, dist_coeffs, rvec[i], tvec[i], 0.05)  # Axes length is set to 5 cm
