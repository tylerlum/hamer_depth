...
def get_first_frame_with_hand(
    imgs_rgb: np.ndarray, detector: DetectorHamer
) -> Tuple[int, dict]:
    """
...
def process_image_with_hamer(
    ...
    T, aligned_hamer_pcd = get_transformation_estimate(
        visible_points_3d, visible_hamer_vertices, pcd
    )
...
def main(args):
    ...
    if args.use_shared:
        videos_folder = os.path.join(
            "/juno/group/human_shadow/raw_data/", args.demo_name
        )
        save_videos_folder = os.path.join(
        ...
        videos_folder = os.path.join(
            root_folder, f"human_shadow/data/videos/{args.demo_name}/"
        )
        save_videos_folder = os.path.join(
            root_folder, f"human_shadow/data/videos/processed/{args.demo_name}/"
        )
        if not os.path.exists(save_videos_folder):
            os.makedirs(save_videos_folder)
    ...
    for video_sub_folder in tqdm(all_video_folders):
        ...
        # Get bbox of arm
        bbox, score = detector_detectron.get_best_bbox(
            imgs_rgb[start_idx], visualize=False
        )

        # Segment out arm in video
        kpts_2d = hamer_output["kpts_2d"]
        masks, sam_imgs = detector_sam.segment_video(
            images_folder,
            bbox=bbox,
            bbox_ctr=kpts_2d.astype(np.int32),
            start_idx=start_idx,
        )
        list_human_masks = [
            (masks[idx][0][0].astype(np.uint8) * 255)
        ...
        for img_idx in tqdm(range(len(image_paths)), leave=False):
            ...
            if img_idx < start_idx:
                ...
                img_annot = np.vstack(
                    (np.hstack((img_rgb, img_sam)), np.hstack((img_hamer, img_pcd)))
                )
            ...
            try:
                finger_pts, img_pcd, img_hamer = process_image_with_hamer(
                    img_rgb, img_depth, mask, cam_intrinsics, detector_hamer, vis
                )
                df.loc[img_idx] = [
                    img_idx,
                    finger_pts["thumb"],
        ...
        media.write_video(
            os.path.join(save_folder, "imgs_pcd.mkv"),
            np.array(list_pcd_imgs),
            fps=10,
            codec="ffv1",
        )
        media.write_video(
            os.path.join(save_folder, "imgs_hamer.mkv"),
            np.array(list_hamer_imgs),
            fps=10,
            codec="ffv1",
        )
        media.write_video(
            os.path.join(save_folder, "imgs_sam.mkv"),
            np.array(sam_imgs),
            fps=10,
            codec="ffv1",
        )
        media.write_video(
            os.path.join(save_folder, "imgs_hand_masks.mkv"),
            list_human_masks,
            codec="ffv1",
            input_format="gray",
        )
        media.write_video(
            os.path.join(save_folder, "imgs_annot.mkv"),
            np.array(list_annot_imgs),
            fps=10,
            codec="ffv1",
        )
