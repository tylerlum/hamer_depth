...
class DetectorSam2:
    ...
    def segment_frame(self, frame: np.ndarray, positive_pts: Optional[np.ndarray]=None,
        ...
        if positive_pts is not None and negative_pts is not None:
            ...
            point_labels = np.concatenate([np.ones(len(positive_pts)), np.zeros(len(negative_pts))], axis=0)
        ...
        if visualize:
            DetectorSam2.show_masks(img, masks, scores, point_coords=point_coords, box_coords=bbox_pts)
    ...
    def show_video_mask(mask: np.ndarray, ax: Axes, obj_id: Optional[int]=None, random_color: bool=False) -> None:
        ...
        if point_coords is not None:
            scatter = ax.scatter(point_coords[:, 0], point_coords[:, 1], color='green', marker='o', 
                    s=5, edgecolor='white', linewidth=1)
        plt.axis('off')
        # plt.show()
...
if __name__ == "__main__":
    ...
    idx = 71
    video_folder = os.path.join(root_folder, f"outputs/demos/video_4/image")
    img_path = os.path.join(root_folder, video_folder, f"000{idx}.jpg")
    frame_bgr = cv2.imread(img_path)
    frame = cv2.cvtColor(frame_bgr, cv2.COLOR_BGR2RGB)
