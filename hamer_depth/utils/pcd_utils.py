...
def get_visible_points(mesh, origin: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    ...
    visible_triangle_indices = intersector.intersects_first(
        np.tile(origin, (pts.shape[0], 1)), directions
    )
...
def radius_outlier_detection(
    points: np.ndarray, radius: float = 5, min_neighbors: int = 5
...
def remove_outliers(
    pcd: o3d.geometry.PointCloud, radius: float = 5, min_neighbors: int = 5
) -> Tuple[o3d.geometry.PointCloud, np.ndarray]:
    """
    Remove outliers from a point cloud using radius-based outlier detection.
    """
    outlier_indices, outlier_pts = radius_outlier_detection(
        np.asarray(pcd.points), radius=radius, min_neighbors=min_neighbors
    )
...
def get_3D_points_from_pixels(
    pixels_2d: np.ndarray, depth_map: np.ndarray, intrinsics: dict
) -> np.ndarray:
    """
